...
- **[实时图表]**：核心的可视化面板已经 100% 竟工。拓扑图采用纯 SVG + DOM 手写渲染（不依赖 React Flow 或 D3），支持流光粒子动画、缩放、平移；底栏图表同样纯 SVG 手绘（温度折线图、风扇仪表盘、24H 在线率柱状图），不依赖 ECharts。
...
- **后台管理与配置面板 (Admin Dashboard)**
  目前整个大屏主要是数据 **消费与展示层**。作为管理员，需要一个能够对系统底层数据进行增删改查的独立后台页面：
  1. 开发路由 `/admin`（已留占位 `Coming Soon`）。
  2. 需要实现：设备管理(CRUD)、别名管理(inline编辑)、OID配置(开关切换)、告警管理(确认+导出)、用户管理。
  3. 详细方案见 `tasks/frontend_plan.md` 第十节和 AI 生成的 Implementation Plan。
