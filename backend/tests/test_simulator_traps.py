...
def _capture_trap(layout="formal", source_host="127.0.0.1"):
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as receiver:
        receiver.bind(("127.0.0.1", 0))
        receiver.settimeout(2)
        host, port = receiver.getsockname()
        send_formal_trap(3, "trap smoke", host, port, source_host=source_host, layout=layout)
...
def test_trap_sender_uses_the_explicit_port_mode_loopback_source_identity():
    address, _ = _capture_trap("formal", source_host="127.0.1.2")

    assert address[0] == "127.0.1.2"
